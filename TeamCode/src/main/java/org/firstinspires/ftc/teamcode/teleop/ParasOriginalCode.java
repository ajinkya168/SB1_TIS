...
@TeleOp(group = "Trial")
@Config
public class ParasOriginalCode extends LinearOpMode {
    ...
    Drone drone = null;
    public static int levelThree = 300, levelTwo = 150, levelOne = 60 ,levelZero = 0, levelFour = 500;
    ...
    sliderState sliderStateInfo = sliderState.LEVEL_ZERO;
    public static int sliderStateValue;
...
//
        ...
        TrajectorySequence outtakePos= drive.trajectorySequenceBuilder(new Pose2d())////
                .addTemporalMarker(()->slider.extendTo(sliderStateValue, output_power))
                .addTemporalMarker(()->Intake.SetIntakePosition(0.55))
                .waitSeconds(0.3)//0.3
                .addTemporalMarker(()->Outtake.setOuttakeArm(0.95))
...
//                sleep(500);
            ...
            if (currentGamepad1.a && !previousGamepad1.a && (!slider_flag || start)) {
...
//                    Outtake.crab.setPosition(0.5);
            ...
            if (currentGamepad1.left_bumper && !previousGamepad1.left_bumper ) {
                if(!outtake_flag){
                    output_power = lifter_pid(kp, ki, kd, sliderStateValue);
                    if (output_power > 0.9) {
                        output_power = 1;
                    } else if (output_power < 0.2) {
                        output_power = 0;
...
//        Hang Motor
            ...
            if(currentGamepad2.a && !previousGamepad2.a){
                sliderStateInfo = sliderState.LEVEL_ONE;
                sliderStateValue = levelOne;
            }
            if(currentGamepad2.a && !previousGamepad2.a){
                sliderStateInfo = sliderState.LEVEL_TWO;
                sliderStateValue = levelTwo;
            }
            if(currentGamepad2.a && !previousGamepad2.a){
                sliderStateInfo = sliderState.LEVEL_THREE;
                sliderStateValue = levelThree;
            }
            if(currentGamepad2.a && !previousGamepad2.a){
                sliderStateInfo = sliderState.LEVEL_FOUR;
                sliderStateValue = levelFour;
            }

            if (currentGamepad2.back && !previousGamepad2.back && (!slider_flag || start)) {
//                intakeToggle != intakeToggle;
                if(!reverse_flag) {
                    Intake.SetIntakePosition(0.55);
                    Intake.IntakeReverse();
...
//        Hanger Down Motor
            ...
            telemetry.addData("SLider State", sliderStateValue);
